...
def apply_rules(text):
    ...
    if re.search(phone_pattern, text):
        ...
        rule_scores['abuse'] += 0.2  # 可��是广告/垃圾信息
...
def main():
    ...
    with tab2:
        ...
        if uploaded_file is not None:
            # 读取 CSV：自动尝试多种常见编码，避免 UnicodeDecodeError
            uploaded_file.seek(0)
            raw = uploaded_file.read()

            encodings_to_try = ['utf-8-sig', 'utf-8', 'gb18030', 'gbk', 'cp1252', 'latin1']
            last_err = None
            df = None
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    st.success(f"CSV 编码识别为: {enc}")
                    break
                except UnicodeDecodeError as e:
                    last_err = e

            if df is None:
                st.error("无法识别 CSV 编码，请确认文件编码（建议另存为 UTF-8）。")
